...
class tripsVC: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource , UICollectionViewDelegateFlowLayout {
    
    var tripsd = [tripsData]()
    ...
    @objc private func handleRefresh() {
        ...
        API_trips.banner(urls: "http://alhabib-abobakr.com/api_elshekh/MainApp/api_token=222&lang=en&section=trips"){ (error: Error?, tripsd: [tripsData]?) in
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView.tag == 0 {
            ...
            if let cells = trips.dequeueReusableCell(withReuseIdentifier: "cell2", for: indexPath) as? tripsCell {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let distantion = segue.destination as? tripsDetialsVC{
            if let prodacet = sender as? tripsData {
